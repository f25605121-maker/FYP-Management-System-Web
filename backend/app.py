...
# Railway reserves DATABASE_URL for linked services, so also check DB_URL as fallback
...
with app.app_context():
    try:
        ...
        print(f"[STARTUP] DB_URL env: {'SET' if os.environ.get('DB_URL') else 'NOT SET'}", flush=True)
        ...
        print(f"[STARTUP] SMTP_PASSWORD env: {'SET' if os.environ.get('SMTP_PASSWORD') else 'NOT SET'}", flush=True)
